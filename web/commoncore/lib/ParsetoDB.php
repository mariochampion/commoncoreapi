...
class ParsetoDB {
	...
	public function chkforexisting_relatediduripreflabel($identity, $iduripref, $tabletocheck ){
		...
		if( 0 ){ echo "<br>tabletocheck $tabletocheck<br>"; }
	...
	public function chkforexisting_relatedliterallanguage($identity, $idliterallanguage, $tabletocheck){
		...
		if( 0 ){ echo "<br>RELATED LITLANG tabletocheck $tabletocheck<br>"; }
			...
			if($numchildrens > 0){
				if( 1 ){ echo "<br>5<br>"; }
					foreach($thisentity['children'] as $child){
						...
						if($importantkey == "asn_comment"){
							if( 1 ){ echo "<br>go asn_comment<br>"; }
							$this->process_asncomment($child['asn_comment'], $child['id']);			
	...
	public function process_edulevel($entity_edulevels, $entity_id){
		...
		foreach($entity_edulevels as $key=>$edulevel){
			...
			if( 0 ){ echo "<br>qryb<pre>";print_r($qryb);echo "</pre>"; }
			...
			if($iduri){
				...
				if( !$this->chkforexisting_relatediduripreflabel($identity, $iduri, 'cc_entitytoedulevels' ) ){
					...
					if( 0 ){ echo "<br>DUPE for (identity,iduripref) $identity, $iduri<br>"; }
				}//end if
	...
	public function process_asnlocalsubject($entity_localsubj, $entity_id){
		...
		if($entity_localsubj_count>0){
			...
			foreach($entity_localsubj as $key=>$edulevel){
				...
				//now add to cc_entitytolocalsubjects
				if($iduri){
	...
	//// functions returns _____
	//// function expects _____
	public function process_asncomment($entity_localsubj, $entity_id){
	
		if( 1 ){ echo "<br>process_asncomment<br>"; }
		//get base vars
		$identity = $this->getidentityfromid($entity_id);	
		if( 1 ){ echo "<h3>--- entity: $entity_id ---</h3><pre>";print_r($entity_localsubj);echo "</pre><br>";}
		...
		if($entity_localsubj_count>0){
			...
			foreach($entity_localsubj as $key=>$edulevel){
				...
				//set up qrys now
				$qryb = "SELECT u.id FROM cc_literalslanguages u WHERE u.etype = 'asn_comment' AND u.literal = '$edu_uri' AND u.language = '$edu_label' LIMIT 1";
				if( 1 ){ echo "<br>qryb<pre>";print_r($qryb);echo "</pre>"; }
				$rsltb = mysql_query($qryb);
				$numb = mysql_num_rows($rsltb);
				...
				if($numb > 0){
					...
					//ADD TO cc_literalslanguages
					$qryi = "INSERT INTO cc_literalslanguages VALUES (NULL, 'asn_comment', '$edu_uri', '$edu_label')";
					$rslti = mysql_query($qryi);
					if(!$rslti){die('noooo! didnt insert new cc_literalslanguages/asn_comment');}
					
					//now go get id again
					$qryb2 = "SELECT u.id FROM cc_literalslanguages u WHERE u.etype = 'asn_comment' AND u.literal = '$edu_uri' AND u.language = '$edu_label' LIMIT 1";
					if( 1 ){ echo "<br>qryb2<pre>";print_r($qryb2);echo "</pre>"; }
					$rsltb2 = mysql_query($qryb2);
					$numb2 = mysql_num_rows($rsltb2);
					if($numb2>0){$iduri = mysql_result($rsltb2, 0, 'id');}
				...
				if($iduri){
					...
					//check for existing record
					if( !$this->chkforexisting_relatedliterallanguage($identity, $iduri, 'cc_entitytoasncomments' ) ){
						
						$qryc = "INSERT INTO cc_entitytoasncomments VALUES (NULL, $identity, $iduri)";
						$rsltc = mysql_query($qryc);
						if( $rsltc ){ echo "&nbsp;&nbsp;&nbsp;SUCCESS: $qryc <br>"; }else{'did NOT insert<br>';}
					...
					//soemthing went wrong!
					die("<h3>2. problem at process_asncomment</h3>");
				}//end if
	...
	//return array (even if empty) of dcterms_educationlevel
	public function getedulevels_byIDentity($identity, $showetype=null){
		
		$identity = mysql_real_escape_string($identity);
		$qry = "SELECT e.iduripref, u.etype, u.uri, u.prefLabel FROM cc_entitytoedulevels e, cc_uripreflabels u WHERE e.identity='$identity' AND u.id = e.iduripref";
			if( 0 ){ echo "<br>getedulevels_byIDentity qry $qry<br>"; }
			$rslt = mysql_query($qry);
			$edulevels_array = array();
			if($rslt){
				$x = 0;
				while($row = mysql_fetch_assoc($rslt)){
					$edulevels_array[$x]['id'] = $row['iduripref'];
					if($showetype){$edulevels_array[$x]['etype'] = $row['etype'];}
					$edulevels_array[$x]['uri'] = $row['uri'];
					$edulevels_array[$x]['preflabel'] = $row['prefLabel'];
					$x++;
				}//end while	
				
				
			}//end if
			if( 0 ){echo "<pre>";print_r($edulevels_array);die();}
			
			return $edulevels_array;
		
	}//end function
	
	
		//return array (even if empty) of dcterms_educationlevel
	public function getedulevels_byIDentity_reduced($identity, $showetype=null){
		
		$identity = mysql_real_escape_string($identity);
		$qry = "SELECT u.prefLabel FROM cc_entitytoedulevels e, cc_uripreflabels u WHERE e.identity='$identity' AND u.id = e.iduripref";
			if( 0 ){ echo "<br>getedulevels_byIDentity qry $qry<br>"; }
			$rslt = mysql_query($qry);
			$edulevels = "";
			if($rslt){
	...
	//return array (even if empty) of skosexactmatches
	public function getskosexactmatch_byIDentity($identity){
		
		$identity = mysql_real_escape_string($identity);
		$qry = "SELECT e.iduripref, u.uri, u.prefLabel FROM cc_skosexactmatches e, cc_uripreflabels u WHERE e.identity='$identity' AND u.id = e.iduripref";
			if( 0 ){ echo "<br>getskosexactmatch_byIDentity qry $qry<br>"; }
			$rslt = mysql_query($qry);
			$skosmatch_array = array();
			if($rslt){
				$x = 0;
				while($row = mysql_fetch_assoc($rslt)){
					$skosmatch_array[$x]['id'] = $row['iduripref'];
					$skosmatch_array[$x]['uri'] = $row['uri'];
					$skosmatch_array[$x]['preflabel'] = $row['prefLabel'];
					$x++;
				}//end while	
				
				
			}//end if
			if( 0 ){echo "<pre>";print_r($skosmatch_array);die();}
			
			return $skosmatch_array;
	...
	//return array (even if empty) of skosexactmatches
	public function getskosexactmatch_byIDentity_reduced($identity){
		
		$identity = mysql_real_escape_string($identity);
		$qry = "SELECT u.uri FROM cc_skosexactmatches e, cc_uripreflabels u WHERE e.identity='$identity' AND u.id = e.iduripref";
			if( 0 ){ echo "<br>getskosexactmatch_byIDentity qry $qry<br>"; }
			$rslt = mysql_query($qry);
			$skosmatch = '';
			if($rslt){
				if(mysql_num_rows($rslt)>0){
					$skosmatch = mysql_result($rslt,0,'uri');
				}
			}//end if
			return $skosmatch;
	...
	//return array (even if empty) of skosexactmatches
	public function getlocalsubjects_byIDentity($identity,$showetype=null){
		
		$identity = mysql_real_escape_string($identity);
		$qry = "SELECT e.idliterallanguage, l.etype, l.literal, l.language FROM cc_entitytolocalsubjects e,  cc_literalslanguages l WHERE e.identity='$identity' AND l.id = e.idliterallanguage";
			if( 0 ){ echo "<br>getlocalsubjects_byIDentity qry $qry<br>"; }
			$rslt = mysql_query($qry);
			$litlang_array = array();
			if($rslt){
				$x = 0;
				while($row = mysql_fetch_assoc($rslt)){
					$litlang_array[$x]['id'] = $row['idliterallanguage'];
					if($showetype){$litlang_array[$x]['etype'] = $row['etype'];}
					$litlang_array[$x]['literal'] = $row['literal'];
					$litlang_array[$x]['language'] = $row['language'];
					$x++;
				}//end while	
				
				
			}//end if
			if( 0 ){echo "<pre>";print_r($litlang_array);die();}
			
			return $litlang_array;
		
	}//end function
	
	
	//return array (even if empty) of skosexactmatches
	public function getasncomment_byIDentity($identity, $showetype=null){
		
		$identity = mysql_real_escape_string($identity);
		$qry = "SELECT e.idliterallanguage, l.etype, l.literal, l.language FROM cc_entitytoasncomments e,  cc_literalslanguages l WHERE e.identity='$identity' AND l.id = e.idliterallanguage";
			if( 0 ){ echo "<br>getasncomment_byIDentity qry $qry<br>"; }
			$rslt = mysql_query($qry);
			$litlang_array = array();
			if($rslt){
				$x = 0;
	...
	//return array (even if empty) of skosexactmatches
	public function getchildren_byIDentity($identity){
		
		$identity = mysql_real_escape_string($identity);
		$qry = "SELECT c.idchild, e.url_final, e.id, e.text, e.asn_identitifier FROM cc_children c, cc_entities e WHERE c.idparent='$identity' AND c.idchild=e.identity";
		if( 0 ){ echo "<br>getchildren_byIDentity qry $qry<br>"; }
		$rslt = mysql_query($qry);
		$children_array = array();
		if($rslt){
			$x = 0;
			while($row = mysql_fetch_assoc($rslt)){
				$children_array[$x]['identity'] = $row['idchild'];
				$children_array[$x]['url_final'] = $row['url_final'];
				$children_array[$x]['asn_uri'] = $row['id'];
				$children_array[$x]['asn_identifier'] = $row['asn_identitifier'];
				$children_array[$x]['text'] = $row['text'];
				$x++;
			}//end while	
			
			
		}//end if
		if( 0 ){echo "<pre>";print_r($children_array);die();}
	...
	//return array (even if empty) of skosexactmatches
	public function getliterallanguage_byIDliteral($idliteral, $etype){
		
		$idliteral = mysql_real_escape_string($idliteral);
		$etype = mysql_real_escape_string($etype);
		
		$qry = "SELECT l.id, l.etype, l.literal, l.language FROM cc_literalslanguages l WHERE l.id='$idliteral' AND l.etype='$etype'";
		if( 0 ){ echo "<br>getliterallanguage_byIDentity qry $qry<br>"; }
		$rslt = mysql_query($qry);
		$litlang_array = array();
		if($rslt){
			$x = 0;
			while($row = mysql_fetch_assoc($rslt)){
				$litlang_array[$x]['id'] = $row['id'];
				//$litlang_array[$x]['etype'] = $row['etype'];
				$litlang_array[$x]['literal'] = $row['literal'];
				$litlang_array[$x]['language'] = $row['language'];
				$x++;
			}//end while	
	...
	//return array (even if empty) of skosexactmatches
	public function geturipref_byIDuripref($iduri, $etype){
		
		$iduri = mysql_real_escape_string($iduri);
		$etype = mysql_real_escape_string($etype);
		
		$qry = "SELECT l.id, l.etype, l.uri, l.preflabel FROM cc_uripreflabels l WHERE l.id='$iduri' AND l.etype='$etype'";
		if( 0 ){ echo "<br>geturipref_byIDuripref qry $qry<br>"; }
		$rslt = mysql_query($qry);
		$uripref_array = array();
		if($rslt){
			$x = 0;
			while($row = mysql_fetch_assoc($rslt)){
				$uripref_array[$x]['id'] = $row['id'];
				//$uripref_array[$x]['etype'] = $row['etype'];
				$uripref_array[$x]['uri'] = $row['uri'];
				$uripref_array[$x]['preflabel'] = $row['preflabel'];
				$x++;
			}//end while	
			
			
		}//end if
		if( 0 ){echo "<pre>";print_r($uripref_array);die();}
		
		return $uripref_array;
		
	}//end function


	//from http://es1.php.net/manual/en/reserved.variables.get.php
